...
namespace Platform
{
	namespace Window {
			...
			renderSurface_()
		{
			...
			renderSurface_ = std::make_unique<Platform::RenderSurface::GLFW_RenderSurface>(window_);
		}

		GLFW_Window::GLFW_Window(int width, int height, const std::string& title, int glMajorVersion, int glMinorVersion)
			: window_(nullptr), width_(width), height_(height), title_(title), shouldClose_(false), initialized_(false), isCleanedUp_(false), renderSurface_()
		{
			if (!glfwInit())
				throw std::runtime_error("Failed to initialize GLFW");
